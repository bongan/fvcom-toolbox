...
%------------------------------------------------------------------------------
% sort all indices elements, nodes or layers to increasing values
%------------------------------------------------------------------------------
nele_idx=sort(nele_idx);
node_idx=sort(node_idx);
siglay_idx=sort(siglay_idx);
siglev_idx=sort(siglev_idx);
%
...
%------------------------------------------------------------------------------
    TF = strcmpi(varnames{aa},vars);varidx(aa) = find(TF);TF = sum(TF);
    dimens=ndims(aa);
    if TF;
            ...
            if sum(do_restrict)
                for dd=1:length(do_restrict)
                    ...
                    if ( do_restrict(dd) & ~(count(dd)==length(idx)) )
                        [dump,idx]=setdiff(start(dd):RestrictDims.idx{dimidx(dd)}(end),RestrictDims.idx{dimidx(dd)})
