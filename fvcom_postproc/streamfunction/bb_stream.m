...
for i=1:nsegs
  ...
  if(seg(i).segname(1:2)=='MV') %MV Dirichlet is set using Naushon
    seg(i).left = 2;
  end;
end;

file = '/Volumes/HYDRA/BUZZBAY/run/CASEA_dply3/output_dply3_4x31/semass_4x31_dply3_lpf_avge_A.nc';  %/Volumes/HYDRA/PROJECTS/BUZZBAY/run/compass_wind/output/scp_4x31_25knot_rosewind_filt_caseB.nc';
file = '/Volumes/Hydra/run/CASEC_dply3/output_dply3_4x31/semass_4x31_dply3_lpf_avge_C.nc';
obcfile = 'scp4.1_obc.dat';
%file = 'box.nc';
%obcfile = 'none';
nc = netcdf(file);
time = nc{'time'}(:); ntimes = numel(time);
frame = 1;
...
for i=2:nsegs-1
  ...
  clear dmin;
  prev = seg(i).left;
  xleft = x(seg(prev).nodes);
  yleft = y(seg(prev).nodes);
  xrght = x(seg(i).nodes);
  yrght = y(seg(i).nodes);
  for j=1:numel(xleft)   
  ...
  fprintf('flux between %s and %s is %f\n',seg(prev).segname,seg(i).segname,flux);
